...
import {
  ...
  type SourceDocumentMaterialInput,
...
export type SourceFactCityNoteRecoveryResultV1 = {
  recoveredCandidateCount: number;
  recoveredCollectionCount: number;
  stages: EvidenceStageInput[];
};
...
function splitSourceFactNoteNames(value: string) {
  return value
    .split(/\s*,\s*|\s+(?:and|or)\s+|\s+\/\s+/i)
    .map(cleanSourceFactNoteTitle)
    .filter((title): title is string => Boolean(title));
}

function labeledRecommendationEntries({
  body,
  evidence,
  subject,
}: {
  ...
  return groups.flatMap((group) => {
    const area = group[1].trim();
    return splitSourceFactNoteNames(group[2]).map((title) => ({
      collectionLabel: `${subject} — ${area}`,
      evidence,
      title,
    }));
  });
...
function sourceFactCityNoteEntries(text: string) {
  const lines = text.replace(/\r\n?/g, "\n").split("\n");
  const entries: SourceFactCityNoteEntry[] = [];
  let activeRecommendationSubject: string | null = null;
  let activeRecommendationLinesRemaining = 0;
  ...
  for (const rawLine of lines) {
    const line = rawLine
      ...
      .trim();
    if (!line) {
      activeRecommendationSubject = null;
      activeRecommendationLinesRemaining = 0;
      continue;
    }
    // Reject flattened PDF text-layer pages. Recovery is only from a short,
    // legible source-authored list line, never a broad document substring.
    if (line.length > 500) {
      activeRecommendationSubject = null;
      activeRecommendationLinesRemaining = 0;
      continue;
    ...
    const popular = /^popular spots for ([^.:]{2,35}) are (.+?)[.!]?$/i.exec(
      line
    );
    if (popular) {
      const subject = popular[1].trim();
      const titles = splitSourceFactNoteNames(popular[2]);
      if (titles.length >= 2 && titles.length <= 12) {
        entries.push(
          ...titles.map((title) => ({
            collectionLabel: `${subject} recommendations`,
            evidence: line,
            title,
          }))
        );
    ...
    const recommendationHeader = /^(.{0,40}\b(?:recs?|recommendations?)\b.{0,120}?)(?:\s+[-—]\s+|:\s*)(.*)$/i.exec(
      line
    );
    if (recommendationHeader) {
      const header = recommendationHeader[1];
      const subject = /\b(food|restaurant|bar|cafe|beer|shopping)\b/i.exec(
        header
      )?.[1] ?? "Local";
      const body = recommendationHeader[2].trim();
      if (body) {
        entries.push(
          ...labeledRecommendationEntries({
            body,
            evidence: line,
            subject: `${subject} recommendations`,
          })
        );
      }
      activeRecommendationSubject = `${subject} recommendations`;
      activeRecommendationLinesRemaining = 6;
      continue;
    }

    const namedCollection =
      /^(eat|food|cafes?|restaurants?|bars?|beer|shopping):\s*(.+)$/i.exec(
        line
      );
    if (namedCollection) {
      const subject = `${namedCollection[1]} recommendations`;
      const titles = splitSourceFactNoteNames(namedCollection[2]);
      if (titles.length >= 2 && titles.length <= 12) {
        entries.push(
          ...titles.map((title) => ({
            collectionLabel: subject,
            evidence: line,
            title,
          }))
        );
        activeRecommendationSubject = subject;
        activeRecommendationLinesRemaining = 3;
      }
      continue;
    }

    if (
      /^you can either\b/i.test(line) &&
      /\bor\b/i.test(line) &&
      /(?:[$€£]\s*\d|\b(?:czk|eur|gbp|huf|usd)\s*\d|\d[\d.,]*\s*(?:czk|eur|gbp|huf|ft|usd)\b)/i.test(
        line
      )
    ) {
      entries.push({
        collectionLabel: "Local transportation",
        description: line,
        evidence: line,
        title: /\bfunicular\b/i.test(line)
          ? "Funicular fare and walking option"
    ...
    if (activeRecommendationSubject && activeRecommendationLinesRemaining > 0) {
      const labeled = /^([\p{L}][\p{L} '&-]{1,34}):\s*(.+)$/u.exec(line);
      const continuationBody = labeled?.[2] ?? line;
      const titles = splitSourceFactNoteNames(continuationBody);
      if (titles.length < 2 && !labeled) {
        activeRecommendationSubject = null;
        activeRecommendationLinesRemaining = 0;
        continue;
      }
      const recommendationSubject = activeRecommendationSubject;
      entries.push(
        ...titles.map((title) => ({
          collectionLabel: labeled
            ? `${recommendationSubject} — ${labeled[1].trim()}`
            : recommendationSubject,
          evidence: line,
          title,
        }))
      );
      activeRecommendationLinesRemaining -= 1;
    }
  }

  return Array.from(
    new Map(
      entries.map((entry) => [normalizeText(entry.title), entry])
    ).values()
  );
}

function uniqueStageCity(stage: EvidenceStageInput) {
  const stageRecord = asRecord(stage.stage);
  const cities = new Set<string>();
  for (const collection of ["activities", "places", "stays", "transport"]) {
    const rows = Array.isArray(stageRecord[collection])
      ? stageRecord[collection]
      : [];
    for (const row of rows) {
      const city = stringValue(asRecord(row), "city");
...
// Deterministic recovery for explicit source-authored named recommendation
// lists. The model is allowed to summarize prose, but a summary may not erase
// named venues that the traveler deliberately put in a list. Only narrow list
// grammar is accepted; arbitrary prose, contact details, directions, and
// flattened PDF pages cannot mint records. Every recovered record retains the
// exact source line and source-span boundary, and enters the City Note lane.
export function recoverMissingSourceFactCityNoteMembersV1({
  index,
  materials,
  stages,
}: {
  ...
  stages: EvidenceStageInput[];
}): SourceFactCityNoteRecoveryResultV1 {
  ...
  for (const stage of stages) {
    ...
    for (const activity of activities) {
      const title = normalizeText(stringValue(asRecord(activity), "title"));
      if (title) existingTitles.add(title);
    }
  }

  const recoveredByStage = new Map<number, Record<string, unknown>[]>();
  const recoveredCollections = new Set<string>();
  for (const material of materials) {
    for (const entry of sourceFactCityNoteEntries(material.text)) {
      const normalizedTitle = normalizeText(entry.title);
      if (!normalizedTitle || existingTitles.has(normalizedTitle)) continue;
      const sourceSpanIds = sourceSpanIdsForMaterialTextV1({
        index,
        material,
        text: entry.evidence,
      });
      ...
      const matchingStages = stages
        .map((stage, stageIndex) => ({
          overlap: (stage.sourceSpanIds ?? []).filter((spanId) =>
            sourceSpanIdSet.has(spanId)
          ).length,
          stage,
          stageIndex,
        }))
        .filter(
          ({ overlap, stage }) =>
            ...
            (material.sourceUploadId
              ? stage.sourceUploadId === material.sourceUploadId
              : stage.sourceFilename === material.filename)
        )
        .sort(
          (left, right) =>
            right.overlap - left.overlap || left.stageIndex - right.stageIndex
        );
      const target = matchingStages[0];
      if (!target) continue;
      const city = uniqueStageCity(target.stage);
      if (!city) continue;
      const sourceSpan = sourceSpanIds
        .map((spanId) => index.lookups.spanById.get(spanId) ?? null)
        .find(Boolean);
      recoveredByStage.set(target.stageIndex, [
        ...(recoveredByStage.get(target.stageIndex) ?? []),
        {
          _sourceFactCityNoteRecovery: true,
          address: null,
          area: null,
          category: /\b(?:food|restaurant|bar|cafe|beer)\b/i.test(
          ...
          date: null,
          description:
            entry.description ?? `${entry.collectionLabel}: ${entry.title}`,
          endTime: null,
          ...
          sourceHeadingPath: [],
          sourceSectionLabel: sourceSpan?.normalizedSectionLabel ?? null,
          sourceSectionType: sourceSpan?.normalizedSectionLabel
            ? "dated_itinerary"
            : "city_reference",
          startTime: null,
          title: entry.title,
      ...
      recoveredCollections.add(
        `${material.sourceUploadId ?? material.filename}|${normalizeText(entry.collectionLabel)}`
      );
    }
  }
  ...
  return {
    ...
    recoveredCollectionCount: recoveredCollections.size,
