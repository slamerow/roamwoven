...
const systemPrompt = [
  ...
  "Calls must be statements, not questions. If you make a non-obvious itinerary call that is worth surfacing but should not block the draft, include it as a missingDetails entry with a statement-style prompt, guessedValue, evidence, confidence, subjectType, and targetField so the app can show it under 'Calls we made' instead of Questions.",
  "For time-bound reservations, pickups, tours, or appointments, decide whether the card has a usable anchor. If it has a specific name, address/location, provider, route, confirmation, or enough descriptive context, create the card and treat missing nice-to-have fields as calls or omissions. If it only has a generic type plus time, such as 'dinner reservation at 6:30' with no restaurant name, address, confirmation, or other identifier, ask a targeted question because the card is not identifiable enough.",
