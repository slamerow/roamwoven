...
const systemPrompt = [
  ...
  "Disjunction rule: when one itinerary slot explicitly offers alternatives ('Lunch at X or Y', 'Modern Art Museum or Design Museum'), emit exactly ONE activity card whose title or description carries the 'X or Y' choice. Never infer 'or' from adjacent source lines, proximity, or venue knowledge, and never also emit the individual alternatives as separate activity cards — that duplicates one decision into several plans.",
  ...
  "Day-section membership rule: every item that sits inside a dated day section belongs to that day even when the line itself has no date — carry the day into the item's date field, and always carry the day heading into sourceSectionLabel. An undated errand or stop sequenced inside a day-trip section ('be back by 5 to go to the shop') belongs to that day, placed by the source's own sequencing, never by which city the venue is in.",
  "Lodging access-instruction rule: lockbox photos, key-pickup steps, buzzer/door/Wi-Fi codes, and arrival directions describe a STAY. Attach them to the stay they describe — matched by lodging name, address, or check-in details — even when those pages sit at the end of the document after other cities' days. Never emit them as a dated activity card, and never attach them to a different city's stay because of page position.",
  "Same-site component rule: when a source day block presents one site visit with component stops ('Prague castle (2 hours): Changing of the Guard 12:00, St. Vitus…' or a Schonbrunn block listing Gloriette, the Palm House, a show, a train pass), name every component stop in the container card's description in source order, in addition to any standalone component cards. The container's description is how downstream grouping knows which stops the visit owns.",
  "Arrival-day recommendation rule: a loose list of venues and experiences under an arrival/travel day ('gypsy music, wine cellar in the Hilton, pastry shop…') with no times, bookings, or first-person commitment is a city-notes dump for that city, not a set of dated activity cards. Emit itemType note / city_note_candidate entries for these, keeping the day only in sourceSectionLabel.",
