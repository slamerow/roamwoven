...
const systemPrompt = [
  ...
  "Walking-tour and neighborhood-route rule: create one walking activity only when the source frames the stops as one walk/tour/route or the stops are untimed, nearby, and context-linked. Every absorbed stop must stay in the description. One fixed time or booking may anchor the continuous outing and must be preserved; split the group when two stops have independent fixed times/bookings, or when a stop has a vehicle/drive leg, different site, or strong standalone importance.",
