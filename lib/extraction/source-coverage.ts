...
const PLANNING_COST_LINE_PATTERNS = [
  ...
  /^(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*\s+\d{1,2}(?:st|nd|rd|th)?\s*,?\s+[a-zà-ÿ .'-]+[-–—]\s*\$\d/i,
...
export function isPlanningCostSectionLabel(label: string | null | undefined) {
  ...
  return (
    ...
    /^(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*\s+\d{1,2}(?:st|nd|rd|th)?\s*,?\s+[a-zà-ÿ .'-]+[-–—]\s*\$\d/i.test(
      trimmed
    )
  );
}
