          ...
          activities: [
            {
              ...
              city: "Paris",
              date: "2032-06-16",
              description: "Go to Route Recovery Museum.",
              evidence: "Go to Route Recovery Museum.",
...
export default async function run() {
  const originalSourceFactShadowEnv =
    process.env.EXTRACTION_FACT_LEDGER_SHADOW;
  ...
  try {
    ...
    if (originalSourceFactShadowEnv === undefined) {
      ...
      process.env.EXTRACTION_FACT_LEDGER_SHADOW = originalSourceFactShadowEnv;
    }
