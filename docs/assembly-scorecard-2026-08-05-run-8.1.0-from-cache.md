...
Trip `4eaf3c6c-f480-442b-8301-c425a032cb87` — 8.1.0. Generated 2026-08-05 by `scripts/scorecard.mjs` in **from-cache** mode. Scope: RW-ORD-001, RW-CLS-001, RW-GRP-001, RW-PLC-001.
...
| State | Count | Meaning |
|---|---:|---|
| FAIL | 0 | Built, and wrong. A defect or a drift. Investigate. |
| NOT CHECKABLE | 0 | The data reaches no surface. An observability defect in its own right. |
| NOT BUILT | 0 | Contract text with no implementation. Expected work, not a defect. |
| PASS | 31 | The contract held. |

Geocode verification lane: **pinned and replayed**.

## RW-ORD-001 — ledger `KNOWN_GAP` — 5 PASS
...
### PASS — `ORD-4` Maker-facing decisions carry a leg-key/date/normalized-title or source anchor alongside the piece id

- Clause: Decision anchors — every maker-facing decision records a stable anchor
- Field read: `report.fingerprints.decisionAnchors[] vs counts.calls + counts.openQuestions`
- Reading: 8/8 maker-facing decision anchor(s) served; 0 malformed

## RW-CLS-001 — ledger `PARTIAL` — 11 PASS
...
- Reading: 31 block decision(s); mixed plan+ideas dates: 2019-01-16, 2019-01-19

### PASS — `CLS-3` Every active City Note is dateless, city-keyed, and owns no leg

- Clause: City Notes are keyed to a city and anchored on its legs; a City Note has no day
- Field read: `records.items[].itemType + .date + .legId + .cityNoteKey`
- Reading: every active note has one city key, no date, and no owning leg

### PASS — `GT-0116-3` Trdelník breakfast is exactly ONE Jan-16 card
...
- Reading: "Trdelník for breakfast"
...
### PASS — `GT-0119-3` The scattered Jan-19 Vienna venues are City Notes unless a stronger planned copy wins
...
- Reading: 4 City Note(s) trip-wide; 1 stronger planned copy/copies correctly win: "St. Stephen’s Cathedral" 2019-01-20 | all non-promoted Jan-19 ideas filed as notes
...
### PASS — `GT-0119-4` Mumok and Natural History remain separate Vienna City Note ideas

- Clause: Separate source lines without commitment markers remain separate City Note ideas; assembly must not invent a disjunction
- Field read: `City Note .description vs records.items[] + records.reviewQuestions[]`
- Reading: both separate source ideas are in City Notes; 0 Activity card(s); 0 open Question(s)
...
## RW-GRP-001 — ledger `KNOWN_GAP` — 9 PASS

### PASS — `GRP-1` Every group child is traceable to source nesting

- Clause: Only source bytes are source evidence; source nesting establishes candidacy, distance only corroborates
- Field read: `records.items[].parentItemId + parent .description/.title`
- Reading: 7 grouped stop(s); all traceable to the container's own text

### PASS — `GRP-2` Every container-context retry is backed by the container's own description

- Clause: The echo rule — a result within ~50 m of the injected container is not evidence
- Field read: `report.extraction.geocodeVerification.candidates[].retryQuery/.containerTitle/.containerSourceSupported`
- Reading: 4 container retry/retries, all source-supported; 5 unlisted retry/retries refused before lookup
