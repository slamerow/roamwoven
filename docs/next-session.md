...
- Central Europe extraction QA improved enough that lodging dates were correct on the first encouraging pass. Current review calibration:
  ...
  - Optional missing-detail rule: if a time-bound reservation, pickup, tour, or appointment has a usable anchor such as a name, address/location, provider, route, confirmation, or enough descriptive context, make the card and treat missing nice-to-have fields as calls/omissions. If it only has a generic type plus time, ask a targeted question because the card is not identifiable enough.
